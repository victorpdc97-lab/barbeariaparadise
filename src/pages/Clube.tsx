          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <div className="text-left">
              ...
              <h1 className="headline-hero text-primary-foreground mb-6 opacity-0 animate-fade-up delay-100">
                Já pensou em<br />
                <span className="italic text-primary-foreground/90">assinar uma barbearia?</span>
              </h1>
              ...
              <button
                ...
                className="btn-primary px-10 py-4 text-base opacity-0 animate-fade-up delay-300"
              >
                Assinar Agora
              </button>
            </div>
            ...
            <div className="relative flex justify-center lg:justify-end opacity-0 animate-fade-up delay-200">
              <img
                ...
                className="max-h-[480px] lg:max-h-[600px] w-auto object-contain drop-shadow-2xl"
              />
            </div>
          </div>
        </div>
        ...
        <div className="container-clean">
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {plans.map((plan, index) => (
              <div
                key={plan.subtitle}
                className={`relative rounded-lg p-8 border-2 transition-all duration-500 opacity-0 animate-fade-up ${
                  plan.featured
                    ? "border-[hsl(var(--gold))] shadow-[0_8px_40px_-12px_hsl(var(--gold)/0.2)]"
              ...
              >
                {plan.featured && (
                  <div className="absolute -top-3 left-6">
                    <span className="inline-flex items-center gap-1.5 px-4 py-1.5 text-xs font-medium tracking-wider uppercase bg-[hsl(var(--gold))] text-foreground rounded-sm">
                      <Crown size={12} />
                      Mais Popular
                    </span>
                  </div>
                ...
                <div className="mb-6 mt-2">
                  <span className="text-xs font-medium px-3 py-1.5 rounded-sm bg-muted text-muted-foreground">
                    {plan.tag}
                  </span>
                </div>
                  ...
                  {plan.options.map((option) => (
                    <div key={option.service} className="flex items-center justify-between py-3 border-b border-border last:border-0">
                      ...
                      <span className="font-display text-lg text-foreground">
                        {option.price}
                        <span className="text-xs font-sans text-muted-foreground">/mês</span>
                      </span>
                    </div>
                  ))}
                </div>
                ...
                <button
                  onClick={handleSubscribe}
                  className={`w-full justify-center ${plan.featured ? "btn-primary" : "btn-outline"} py-4`}
                >
                  Assinar
                </button>
              </div>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <div className="opacity-0 animate-fade-up">
              ...
              <h2 className="headline-section text-foreground mb-8">
                Vantagens de<br />
                <span className="italic">ser membro</span>
              </h2>
            ...
            <div className="opacity-0 animate-fade-up delay-200">
              <div className="card-clean p-8">
                <p className="text-muted-foreground text-sm mb-5 font-medium">
                  Benefícios em +30 mil estabelecimentos parceiros
                </p>
                <div className="flex flex-wrap gap-2">
                  {partners.map((partner) => (
                    <span key={partner} className="px-3 py-1.5 bg-background rounded-sm text-xs text-foreground border border-border">
                      {partner}
                    </span>
                  ))}
                </div>
            ...
            <Accordion type="single" collapsible className="space-y-3">
              {faqs.map((faq, index) => (
                <AccordionItem
                  key={index}
                  value={`item-${index}`}
                  className="border border-border rounded-lg px-6 opacity-0 animate-fade-up"
                  style={{ animationDelay: `${(index + 2) * 0.06}s` }}
                >
                  <AccordionTrigger className="text-foreground text-sm font-medium hover:text-[hsl(var(--gold))] hover:no-underline py-4">
                    {faq.question}
                  </AccordionTrigger>
      ...
      <section className="section-clean bg-foreground">
        <div className="container-clean">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <h2 className="font-display text-3xl md:text-4xl text-primary-foreground mb-4 opacity-0 animate-fade-up delay-100">
              Assine pelo app
            </h2>

            <p className="text-primary-foreground/60 text-sm mb-8 opacity-0 animate-fade-up delay-200">
              Faça sua assinatura e gerencie seu plano diretamente pelo aplicativo
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center opacity-0 animate-fade-up delay-300">
              <a
                ...
                className="inline-flex items-center justify-center gap-3 px-6 py-3.5 bg-primary-foreground/10 text-primary-foreground rounded-sm hover:bg-primary-foreground/20 transition-all duration-300 border border-primary-foreground/10"
              >
                <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current">
                  <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
                </svg>
                <div className="text-left">
                  <div className="text-[10px] opacity-60">Disponível na</div>
              ...
              >
                ...
                <div className="text-left">
                  <div className="text-[10px] opacity-60">Disponível no</div>
